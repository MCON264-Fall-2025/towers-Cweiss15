...
> Base case that stops recursion is if there are no more discs to move to the destination so base would be n = 0.


### 2. Recursive Case
_Explain the sequence of recursive calls and what each represents._

> In the recursion call we move n-1 to auxilary using destination. (Then move 1 to destination). Then recursively move n-1 to destination using start.
...
>  I changed the middle line between the recursive calls to increase the count instead of print the move.
...
> The move placements aren't recorded just total number of moves. They accomplish two different things, the first is easier to visualize, the second is more efficient.
...
> It makes total number of moves roughly 3n^2-1 vs 2n^2-1 so becomes much larger very fast.
...
> Because moves that could be done more simply now require extra steps. Roughly increases exponent by one.
...
> Java stack size has a limited amount of memory and recursive calls take up space on the stack. According to Google probably 2000-3000 discs but seemed to run for me just would take a long time.
